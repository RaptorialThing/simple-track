...
use Longman\TelegramBot\Entities\Logistics\Work;
use PDO;
use PDOException;
...
class DB
{
    ...
    public static function insertWorker(Worker $worker): bool 
    {
        if (!self::isDbConnected()) {
            return false;
        }
    ...
    try {
        ...
        $sth = self::$pdo->prepare('
            INSERT INTO `'.TB_WORKERS.'`(`name`,`address`,`status_is_free`,`contact_phone`, `registration_date`)
            VALUES
            (:name, :address, :status_is_free, :contact_phone, :registration_date)
            ...
            $sth->bindValue(':name', $worker->getter('name'));
            $sth->bindValue(':address', $worker->getter('address'));
            $sth->bindValue(':status_is_free',$worker->getter('statusIsFree'));
            $sth->bindValue(':contact_phone', $worker->getter('phone'));
            $sth->bindValue(':registration_date', $date);
    ...
    public static function getWorker($worker_id)
    {
        ...
        try {
            $sth = self::$pdo->prepare('SELECT
                (SELECT `name`,`address`,`status_is_free`,`contact_phone` FROM `' . TB_WORKERS . '` WHERE `id` = :id)
            ');

            $sth->bindValue(':id', $worker_id);
    ...
    public static function getWorkerByPhone($worker_phone)
    {
        if (!self::isDbConnected()) {
            return false;
        }
        ...
        try {
            $sth = self::$pdo->prepare('SELECT
                (SELECT `name`,`address`,`status_is_free`,`contact_phone` FROM `' . TB_WORKERS . '` WHERE `contact_phone` LIKE :phone)
            ');
